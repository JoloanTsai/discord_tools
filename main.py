...
async def get_day_summary_text(channel_id:str, guild_id:int):
    await get_channels_info_and_save(client, select_guild_ids=[guild_id])
...
async def get_rag_query_text(input_text:str, guild_id:int):
    await get_channels_info_and_save(client, select_guild_ids=[guild_id])
    await save_chat(client, guild_ids=guild_id, print_output_info=False)
...
async def rag_query(interaction:discord.Interaction):
    ...
    output_text = await get_rag_query_text(input_text, guild_id)
